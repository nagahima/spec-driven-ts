...
| 仕様 | 理由 |
|-----|------|
| `spec-management` | バグ分析で `Spec` を参照する（AC-01）。仕様欠陥時に `SpecChangeProposal` を生成して差し戻す（AC-04） |
...
- [ ] AC-02: 実装のバグと判定された場合、修正内容を記述した修正タスクが生成される
- [ ] AC-03: 生成された修正タスクが完了すれば、バグ報告が `resolved` になる
...
| コマンド | 説明 |
|---------|------|
| `sdd bug report "[description]"` | バグを報告して分析・タスク生成を開始 |
| `sdd bug list` | バグ報告の一覧とステータスを表示 |
| `sdd bug show [id]` | 分析結果・生成タスクの詳細を表示 |

## 非機能要件

- **透明性**: 分析結果と根拠は必ず `sdd bug show` で確認できる状態で記録する
- **パフォーマンス**: バグ分析・タスク生成を 10秒以内に完了する
- **トークン効率**: バグ分析時は関連仕様のみをコンテキストに渡す。全仕様の一括読み込みは行わない
- **エクスポート**: バグ報告・分析結果・修正タスクの内容はMarkdown形式で出力できる
